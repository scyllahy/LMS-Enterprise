...
if (!client.includes('startExamSecurity()') || !client.includes("exam.security-event") || !client.includes('requestExamFullscreen()') || !client.includes('visibilitychange')) { failed = true; console.error('Student exam must enable fullscreen anti-cheat monitoring'); }
if (!client.includes('state.exam.resumed') || !fs.readFileSync(path.join(src, '18_ExamEngine.gs'), 'utf8').includes('examPayload_(active,p,true)')) { failed = true; console.error('Active student attempts must resume instead of blocking the exam'); }
if (!client.includes("server('exam.save-answers'") || !fs.readFileSync(path.join(src, '36_ApiGateway.gs'), 'utf8').includes("'exam.save-answers'")) { failed = true; console.error('Exam submission must batch-save answers in one request'); }
if (!fs.readFileSync(path.join(src, '21_ScoringService.gs'), 'utf8').includes('published:!pending') || !fs.readFileSync(path.join(src, '46_PortalService.gs'), 'utf8').includes('scoreVisibleToLearner_')) { failed = true; console.error('Fully auto-graded scores must be visible to students'); }
if (!client.includes('riskEventCount') || !fs.readFileSync(path.join(src, '46_PortalService.gs'), 'utf8').includes('securityEventCounts')) { failed = true; console.error('Risk display must count security events'); }
