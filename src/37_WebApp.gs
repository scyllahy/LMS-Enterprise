function doGet(e){if(e&&e.parameter&&String(e.parameter.action||'').toLowerCase()==='health')return out_(ApiGateway.handle({action:'public.health'}));return HtmlService.createTemplateFromFile('Index').evaluate().setTitle(APP_CONFIG.app.name)}
